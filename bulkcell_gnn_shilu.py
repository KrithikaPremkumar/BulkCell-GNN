...
class ShiLUSwishTanh(nn.Module):
    """
    Learnable activation function: f(x; α, γ, δ) = α·[x·σ(γ(x+δ))] + (1-α)·tanh(x)

    Combines a shifted Swish-like gated term (unbounded, adaptive) with a tanh
    term (bounded, symmetric). Three learnable scalar parameters per instance:
        alpha: blend coefficient between ShiLU and tanh terms (init 0.5)
        gamma: sigmoid gate steepness (init 1.0)
        delta: input shift before gating (init 0.0)
    """
    def __init__(self, alpha_init=0.5, gamma_init=1.0, delta_init=0.0):
        super(ShiLUSwishTanh, self).__init__()
        self.alpha = nn.Parameter(torch.tensor(alpha_init))
        self.gamma = nn.Parameter(torch.tensor(gamma_init))
        self.delta = nn.Parameter(torch.tensor(delta_init))

    def forward(self, x):
        shilu_part = x * torch.sigmoid(self.gamma * (x + self.delta))
        tanh_part  = torch.tanh(x)
        return self.alpha * shilu_part + (1 - self.alpha) * tanh_part
...
# Utility: call this after instantiating a model to verify ShiLU parameter overhead
def count_shilu_params(model):
    total = sum(p.numel() for p in model.parameters())
    shilu = sum(
        p.numel()
        for m in model.modules()
        if isinstance(m, ShiLUSwishTanh)
